...
def lint_code():
    ...
    # Run ruff
    if not run_command(
        [sys.executable, "-m", "ruff", "check", "commandrex/"], "Running ruff linter"
    ):
        success = False

    # Run black check
    if not run_command(
        [sys.executable, "-m", "black", "--check", "commandrex/"],
...
def format_code():
    ...
    return run_command(
        [sys.executable, "-m", "black", "commandrex/"], "Formatting code with black"
    )
