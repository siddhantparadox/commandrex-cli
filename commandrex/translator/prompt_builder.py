...
class PromptBuilder:
    ...
    def __init__(self):
        ...
        self.platform_shell_overrides = {
            "windows": {
                "bash": (
                    "The user is using Git Bash on Windows. Use Unix/Bash commands, NOT PowerShell or CMD commands.\n"
...
"""
    ...
    def build_explanation_prompt(self, command: str) -> List[Dict[str, str]]:
        ...
        [Command: "find . -name "*.txt" -type f -size +1M"]
        {
          ...
          "components": [
            {"part": "find", "description": "The command that searches for files in a directory hierarchy"},
            {"part": ".", "description": "The starting directory for the search (current directory)"},
            {"part": "-name \"*.txt\"", "description": "Matches files with names ending in .txt"},
            {"part": "-type f", "description": "Restricts the search to regular files (not directories or other special files)"},
    ...
    def build_safety_assessment_prompt(self, command: str) -> List[Dict[str, str]]:
        ...
        [Command: "rm -rf /"]
        {
          ...
          "risk_level": "high",
          "concerns": [
            "This command attempts to recursively delete all files from the root directory",
