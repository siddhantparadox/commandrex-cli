        ...
        if reset_api_key:
            ...
            if set_new_key:
                ...
                if not api_manager.is_api_key_valid(api_key):
                    ...
                    console.print(
                        "You will be prompted to enter an API key the next time you run CommandRex."
                    )
                    raise typer.Exit(1)
...
@app.command()
def explain(
    command: List[str] = typer.Argument(None, help="Command to explain."),
    api_key: Optional[str] = typer.Option(
        None, "--api-key", help="OpenAI API key (overrides stored key)."
    ),
    model: str = typer.Option(
...
def run(
    ...
    if query:
        ...
        if debug:
            ...
            if shell_info:
                ...
                console.print(
                    f"  • Git Bash detection: {platform_utils.is_windows() and shell_info[0] == 'bash'}"
                )
            else:
    ...
    if debug:
        ...
        console.print(
            f"  • Shell: {platform_info.get('shell_name', 'Unknown')} {platform_info.get('shell_version', '')}"
        )
