...
class TestCLIEndToEnd:
    ...
    def test_translate_command_no_api_key(self, mock_check_api_key, mock_get_api_key):
        ...
        mock_check_api_key.return_value = (
            False  # Simulate user declining to set up API key
        )
    ...
    def test_run_command_with_execution(
        self, mock_shell_manager_class, mock_client_class, mock_get_api_key
    ):
        ...
        mock_shell_manager.execute_command_safely = AsyncMock(
    ...
    def test_run_command_dangerous_blocked(self, mock_client_class, mock_get_api_key):
        ...
        assert (
            result.exit_code == 0
        )  # Command translates but doesn't execute due to danger
...
class TestCLIConfiguration:
    ...
    def test_api_key_setup_interactive(self, mock_input, mock_save_api_key):
        ...
        with patch("commandrex.config.api_manager.get_api_key", return_value=None):
            with patch("commandrex.main.typer.confirm", return_value=True):
                with patch(
                    "commandrex.main.typer.prompt",
                    return_value="sk-test123456789012345678901234567890123456789012",
                ):
                    with patch(
                        "commandrex.main.api_manager.is_api_key_valid",
                        return_value=True,
                    ):
                        with patch(
                            "commandrex.main.api_manager.save_api_key",
                            return_value=True,
                        ):
    ...
    def test_debug_mode_flag(self):
        ...
        with patch(
            "commandrex.config.api_manager.get_api_key",
            return_value="sk-test123456789012345678901234567890123456789012",
        ):
            with patch(
                "commandrex.translator.openai_client.OpenAIClient"
                ...
                with patch("commandrex.main.check_api_key", return_value=True):
                    # Use run command with --translate option since --debug is only
                    # available on run command
                    result = self.runner.invoke(
                        app, ["run", "--debug", "--translate", "list files"]
                    )
...
class TestCLIErrorHandling:
    ...
    def test_invalid_json_response(self, mock_client_class, mock_get_api_key):
        ...
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        mock_client.translate_to_command = AsyncMock(
...
class TestCLIIntegrationWorkflows:
    ...
    def test_platform_specific_translation(
        self, mock_platform_info, mock_client_class, mock_get_api_key
    ):
    ...
    def test_command_with_alternatives(self, mock_client_class, mock_get_api_key):
        ...
        assert (
            "alternative" in result.stdout.lower()
            or "other options" in result.stdout.lower()
        )

    @patch("commandrex.config.api_manager.get_api_key")
    @patch("commandrex.translator.openai_client.OpenAIClient")
    @patch("commandrex.executor.shell_manager.ShellManager")
    def test_full_workflow_translate_and_execute(
        self, mock_shell_manager_class, mock_client_class, mock_get_api_key
    ):
        ...
        assert (
            "/home/user/projects" in result.stdout
            or "executed successfully" in result.stdout.lower()
        )
    ...
    def test_yes_flag_skips_confirmation(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "commandrex.executor.shell_manager.ShellManager"
                ) as mock_shell_manager_class:
                    # Setup mocks
                    mock_client = Mock()
                    mock_client_class.return_value = mock_client
                    ...
                    mock_client.assess_command_safety = AsyncMock(
