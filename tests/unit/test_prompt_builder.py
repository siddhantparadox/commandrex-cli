...
class TestPlatformPromptGeneration:
    ...
    def test_get_platform_prompt_windows_git_bash(self, mock_detect_shell, mock_is_windows):
    ...
    @patch('commandrex.translator.prompt_builder.platform_utils.is_windows')
    @patch('commandrex.translator.prompt_builder.platform_utils.detect_shell')
    def test_get_platform_prompt_windows_standard(self, mock_detect_shell, mock_is_windows):
    ...
    def test_get_platform_prompt_linux(self, mock_is_linux, mock_is_macos, mock_is_windows):
    ...
    @patch('commandrex.translator.prompt_builder.platform_utils.is_macos')
    @patch('commandrex.translator.prompt_builder.platform_utils.is_linux')
    def test_get_platform_prompt_unknown(self, mock_is_linux, mock_is_macos, mock_is_windows):
...
class TestShellPromptGeneration:
    ...
    @patch('commandrex.translator.prompt_builder.platform_utils.detect_shell')
    @patch('commandrex.translator.prompt_builder.platform_utils.is_windows')
    def test_get_shell_prompt_git_bash_on_windows(self, mock_is_windows, mock_detect_shell):
    ...
    def test_get_shell_prompt_zsh(self, mock_detect_shell):
        ...
        mock_detect_shell.return_value = ("zsh", "5.8", {
            "supports_redirection": True,
            "array_support": True
        })
    ...
    def test_get_shell_prompt_fish(self, mock_detect_shell):
        ...
        mock_detect_shell.return_value = ("fish", "3.1", {
            "supports_redirection": True,
...
class TestSystemContextBuilding:
    ...
    def test_build_system_context_with_platform_info(
        self, mock_detect_shell, mock_get_terminal_size, 
        mock_supports_ansi_colors, mock_get_platform_info
    ):
...
class TestPlatformExamples:
    ...
    @patch('commandrex.translator.prompt_builder.platform_utils.is_macos')
    @patch('commandrex.translator.prompt_builder.platform_utils.is_linux')
    def test_get_platform_examples_linux(self, mock_is_linux, mock_is_macos, mock_is_windows):
...
class TestTranslationPromptBuilding:
    ...
    @patch('commandrex.translator.prompt_builder.platform_utils.detect_shell')
    @patch('commandrex.translator.prompt_builder.platform_utils.is_windows')
    def test_build_translation_prompt_git_bash_special_case(self, mock_is_windows, mock_detect_shell):
    ...
    def test_build_translation_prompt_with_shell_examples(self, mock_detect_shell):
        ...
        mock_detect_shell.return_value = ("powershell", "5.1", {"object_pipeline": True})
...
class TestPromptBuilderIntegration:
    ...
    def test_full_prompt_building_workflow(
        ...
        assert len(translation_messages) >= 5  # System, context, examples, history, preferences, user
        
        # Test explanation prompt
        explanation_messages = builder.build_explanation_prompt("Get-ChildItem -Recurse")
...
class TestPromptBuilderEdgeCases:
    ...
    def test_none_command_history(self):
        ...
        history_found = any("Recent command history" in msg.get("content", "") for msg in messages)
    ...
    def test_none_user_preferences(self):
        ...
        pref_found = any("User preferences" in msg.get("content", "") for msg in messages)
    ...
    def test_very_long_command_history(self):
        ...
        messages = builder.build_translation_prompt("list files", command_history=long_history)
...
class TestPromptBuilderConstants:
    ...
    def test_platform_prompts_completeness(self):
        ...
        for platform in required_platforms:
            ...
            assert platform.title() in prompt or platform.upper() in prompt or "macOS" in prompt
    ...
    def test_shell_prompts_completeness(self):
        ...
        for shell in required_shells:
            ...
            assert shell.title() in prompt or shell.upper() in prompt or shell in prompt or "PowerShell" in prompt
    ...
    def test_safety_prompt_content_quality(self):
        ...
        safety_keywords = [
            "SAFETY", "Never generate commands", "harm", "delete files",
            "warnings", "dangerous", "safer alternatives", "explain"
        ]
    ...
    def test_base_system_prompt_json_format(self):
        ...
        json_fields = [
            "command", "explanation", "safety_assessment",
            "is_safe", "concerns", "risk_level", "components",
            "is_dangerous", "alternatives"
        ]
