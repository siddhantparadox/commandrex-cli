...
from commandrex.main import (
    app, get_version, check_api_key, process_translation,
    translate, explain, run, callback
)
...
class TestApiKeyChecking:
    ...
    def test_check_api_key_invalid_existing_key_reset(
        self, mock_prompt, mock_confirm, mock_save, mock_delete, mock_is_valid, mock_get_key
    ):
...
class TestTranslateCommand:
    ...
    def test_translate_invalid_api_key_option(self, mock_is_valid):
        ...
        result = self.runner.invoke(app, ["translate", "--api-key", "invalid", "test query"])
    ...
    def test_translate_success(self, mock_asyncio_run, mock_pb_class, mock_client_class, mock_check_key):
    ...
    @patch('commandrex.main.openai_client.OpenAIClient')
    @patch('commandrex.main.asyncio.run')
    def test_translate_api_error(self, mock_asyncio_run, mock_client_class, mock_check_key):
...
class TestExplainCommand:
    ...
    def test_explain_success(self, mock_get_loop, mock_analyzer_class, mock_client_class, mock_check_key):
    ...
    @patch('commandrex.main.security.CommandSafetyAnalyzer')
    @patch('commandrex.main.asyncio.get_event_loop')
    def test_explain_dangerous_command(self, mock_get_loop, mock_analyzer_class, mock_client_class, mock_check_key):
        ...
        mock_result = {
            ...
            "components": [{"part": "rm -rf", "description": "force remove recursively"}],
            "examples": [],
...
class TestRunCommand:
    ...
    def test_run_with_translate_option(self, mock_is_windows, mock_detect_shell, mock_process):
        ...
        assert result.exit_code == 0
        mock_process.assert_called_once_with("list files", None, "gpt-4o-mini", yes_flag=False)
    ...
    def test_run_platform_detection_calls(self, mock_is_windows, mock_detect_shell):
        ...
        with patch('commandrex.main.process_translation'):
            result = self.runner.invoke(app, ["run", "--translate", "test"])
...
class TestProcessTranslation:
    ...
    @patch('commandrex.main.asyncio.run')
    @patch('commandrex.main.typer.confirm')
    def test_process_translation_no_execute(self, mock_confirm, mock_asyncio_run, mock_get_key, mock_pb_class, mock_client_class):
    ...
    def test_process_translation_with_execute(
        self, mock_confirm, mock_asyncio_run, mock_shell_class, mock_get_key, mock_pb_class, mock_client_class
    ):
        ...
        mock_shell.execute_command_safely = AsyncMock(return_value=(mock_exec_result, None))
    ...
    @patch('commandrex.main.prompt_builder.PromptBuilder')
    @patch('commandrex.main.asyncio.run')
    def test_process_translation_api_error(self, mock_asyncio_run, mock_pb_class, mock_client_class):
        ...
        # Capture console output
        with patch('commandrex.main.console') as mock_console:
            process_translation("test query", "sk-test123", "gpt-4o-mini", yes_flag=False)
            
            # Verify error was printed - check if console.print was called with error message
...
class TestMainEdgeCases:
    ...
    @patch('commandrex.main.typer.confirm')
    @patch('commandrex.main.typer.prompt')
    def test_check_api_key_save_failure(self, mock_prompt, mock_confirm, mock_is_valid, mock_save):
