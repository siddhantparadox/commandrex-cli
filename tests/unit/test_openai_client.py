...
from commandrex.translator.openai_client import OpenAIClient, CommandTranslationResult
...
class TestOpenAIClientInitialization:
    ...
    def test_client_initialization_with_api_key(self, mock_async_openai, mock_api_manager):
        """Test client initialization with provided API key."""
        mock_api_manager.is_api_key_valid.return_value = True
        
        client = OpenAIClient(api_key="sk-test123456789012345678901234567890123456789012345678")
        
        assert client.api_key == "sk-test123456789012345678901234567890123456789012345678"
        assert client.model == "gpt-4o-mini"
        assert client.min_request_interval == 0.5
        mock_async_openai.assert_called_once_with(api_key="sk-test123456789012345678901234567890123456789012345678")
    
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    def test_client_initialization_from_keyring(self, mock_async_openai, mock_api_manager):
        """Test client initialization with API key from keyring."""
        mock_api_manager.get_api_key.return_value = "sk-keyring123456789012345678901234567890123456789012345"
        mock_api_manager.is_api_key_valid.return_value = True
        
        client = OpenAIClient()
        
        assert client.api_key == "sk-keyring123456789012345678901234567890123456789012345"
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    def test_client_initialization_custom_model(self, mock_async_openai, mock_api_manager):
...
class TestRateLimiting:
    ...
    async def test_rate_limit_delay(self, mock_sleep, mock_time, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.time')
    @patch('commandrex.translator.openai_client.asyncio.sleep')
    async def test_rate_limit_no_delay_needed(self, mock_sleep, mock_time, mock_async_openai, mock_api_manager):
...
class TestTranslateToCommand:
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_success(self, mock_async_openai, mock_api_manager):
        ...
        mock_response.choices[0].message.content = json.dumps({
            "command": "ls -la",
            "explanation": "List files with details",
            "safety_assessment": {"is_safe": True, "risk_level": "low", "concerns": []},
            "components": [{"part": "ls", "description": "list command"}],
            "is_dangerous": False,
            "alternatives": ["dir"]
        })
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_with_streaming(self, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_json_decode_error(self, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_rate_limit_error(self, mock_async_openai, mock_api_manager):
        ...
        mock_client = Mock()
        mock_client.chat.completions.create = AsyncMock(side_effect=RateLimitError("Rate limit exceeded", response=mock_response, body=None))
        mock_async_openai.return_value = mock_client
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_api_error(self, mock_async_openai, mock_api_manager):
        ...
        mock_request = Mock()
        mock_client.chat.completions.create = AsyncMock(side_effect=APIError("API error", request=mock_request, body=None))
        mock_async_openai.return_value = mock_client
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_http_error(self, mock_async_openai, mock_api_manager):
        """Test handling of HTTP errors."""
        mock_api_manager.is_api_key_valid.return_value = True
        
        mock_client = Mock()
        mock_client.chat.completions.create = AsyncMock(side_effect=httpx.HTTPError("Network error"))
        mock_async_openai.return_value = mock_client
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_translate_to_command_unexpected_error(self, mock_async_openai, mock_api_manager):
        ...
        with patch.object(client, '_handle_rate_limit', new_callable=AsyncMock):
            with pytest.raises(ValueError, match="Unexpected error during command translation"):
                await client.translate_to_command("test", {})
...
class TestExplainCommand:
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_explain_command_json_error(self, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_explain_command_general_error(self, mock_async_openai, mock_api_manager):
...
class TestAssessCommandSafety:
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_assess_command_safety_success(self, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_assess_command_safety_dangerous(self, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_assess_command_safety_json_error(self, mock_async_openai, mock_api_manager):
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_assess_command_safety_general_error(self, mock_async_openai, mock_api_manager):
...
class TestOpenAIClientEdgeCases:
    ...
    @patch('commandrex.translator.openai_client.api_manager')
    @patch('commandrex.translator.openai_client.AsyncOpenAI')
    async def test_streaming_with_none_content(self, mock_async_openai, mock_api_manager):
        ...
        mock_chunk2.choices[0].delta.content = '{"command": "ls", "explanation": "test", "safety_assessment": {}, "components": [], "is_dangerous": false}'
